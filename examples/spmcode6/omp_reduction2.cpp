...
void merge_hash(std::unordered_map<int, int>& out,
                const std::unordered_map<int, int>& in)
{
...
// Syntax: reduction(identifie: type-list : combiner)
// initializer(initializer-expression)
#pragma omp declare reduction(                                                 \
        hash_merge : std::unordered_map<int, int> : merge_hash(                \
                omp_out, omp_in)) initializer(omp_priv = omp_orig)

int main()
{
    const int N = 20000;
    // global_map will be the reduction variable that collects counts from all
    // iterations

...
public:
    ...
    inline bool push(T* const data)
    {
        ...
        { // CAS loop
            ...
            if (pw == seq)
            {
                if (pwrite.compare_exchange_weak(pw, pw + 1,
    ...
    inline bool pop(T*& data)
    {
        ...
        { // CAS loop
            ...
            if (diff == 0)
            {
                if (pread.compare_exchange_weak(pr, (pr + 1),
                                                std::memory_order_relaxed))
                    break;
                // exponential delay with max value
                for (unsigned i = 0; i < bk; ++i)
...
void test_lockfree_queue(const int nprod, const int ncons, const int size,
                         const int numiterations, const int min, const int max)
{
    ...
    std::cout << "Lock-free queue: Produced " << produced.load()
              << ", Consumed " << consumed.load() << " in " << elapsed.count()
              << " seconds.\n";
}

void test_blocking_queue(const int nprod, const int ncons, const int size,
                         const int numiterations, const int min, const int max)
{
    ...
    std::cout << "Lock-based queue: Produced " << produced.load()
              << ", Consumed " << consumed.load() << " Time " << elapsed.count()
              << " s\n";
}
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 4)
    {
        std::printf("Usage: %s size #prod #cons [numiter=%d] [workmin=%dns] "
                    "[workmax=%dns]\n",
                    argv[0], numiterations, random_work_min, random_work_max);
        return -1;
    }
    ...
    if (argc > 6)
        random_work_min = std::stol(argv[6]);
    ...
    test_lockfree_queue(nprod, ncons, size, numiterations, random_work_min,
                        random_work_max);
    test_blocking_queue(nprod, ncons, size, numiterations, random_work_min,
                        random_work_max);

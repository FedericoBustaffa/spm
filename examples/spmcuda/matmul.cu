...
//    C: (M x N)
//
__global__ void mmTiled(const float *A, const float *B, float *C, int M, int N, int L)
{
...
__global__ void mm(const float *A, const float *B, float *C, int M, int N, int L)
{
	int row = blockIdx.y * BLOCK_SIZE + threadIdx.y;
	int col = blockIdx.x * BLOCK_SIZE + threadIdx.x;
	if (row < M && col < N)
...
void checkResult(float *C, float *Cc, uint64_t M, uint64_t N)
{
...
int main(int argc, char *argv[])
{
	...
	cudaMemcpy(dA, hA, sizeA, H2D);
	CUERR;
	cudaMemcpy(dB, hB, sizeB, H2D);
	CUERR;
	...
	TIMERSTART(mm_tiled);
	// launch the kernel
	mmTiled<<<grid, block>>>(dA, dB, dC, M, N, L);
	CUERR;
	...
	// copy back C matrix
	cudaMemcpy(hC, dC, sizeC, D2H);
	CUERR;

	TIMERSTART(mm_naive);
	mm<<<grid, block>>>(dA, dB, dC, M, N, L);
	CUERR;
	cudaDeviceSynchronize();
	CUERR;
	TIMERSTOP(mm_naive);

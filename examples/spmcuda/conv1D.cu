...
int main()
{
	...
	cudaMalloc((void **)&d_input, (N + 2) * sizeof(float));
	CUERR;
	cudaMalloc((void **)&d_output, N * sizeof(float));
	CUERR;

	// Copy input data from host to device
	cudaMemcpy(d_input, h_input, (N + 2) * sizeof(float), H2D);
	CUERR;
	...
	naive_conv1D<<<numBlocks, THREADS_PER_BLK>>>(N, d_input, d_output);
	CUERR;
	cudaDeviceSynchronize(); // wait for the kernel to finish
	TIMERSTOP(naive_conv1D);

	TIMERSTART(conv1D);
	conv1D<<<numBlocks, THREADS_PER_BLK>>>(N, d_input, d_output);
	CUERR;
	cudaDeviceSynchronize(); // wait for the kernel to finish
	TIMERSTOP(conv1D);
	...
	cudaMemcpy(h_output, d_output, N * sizeof(float), D2H);
	CUERR;
	TIMERSTOP(copy_back);

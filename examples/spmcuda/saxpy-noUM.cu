...
void checkResult(const float *x, const float *y, const float a, int n, const float *y2)
{
	for (int i = 0; i < n; ++i)
	{
		float v = a * x[i] + y[i];
...
int main()
{
	...
	cudaMemcpy(d_x, x, N * sizeof(float), H2D);
	CUERR;
	cudaMemcpy(d_y, y, N * sizeof(float), H2D);
	CUERR;
	...
	cudaMemcpy(y, d_y, N * sizeof(float), D2H);
	CUERR;

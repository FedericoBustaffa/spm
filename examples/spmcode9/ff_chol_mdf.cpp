...
{
#if !defined(DO_NOTHING)
    cblas_cherk(CblasRowMajor, CblasLower, CblasNoTrans, bs, bs, alpha.r,
...
void F_CGEMM(float_complex_t* A, float_complex_t* B, float_complex_t* C,
             float_complex_t*)
{
#if !defined(DO_NOTHING)
    cblas_cgemm(CblasRowMajor, CblasNoTrans, CblasConjTrans, bs, bs, bs,
                (const void*)&alpha,
...
{
#if !defined(DO_NOTHING)
    cblas_ctrsm(CblasRowMajor, CblasRight, CblasLower, CblasConjTrans,
                CblasNonUnit, bs, bs, (const void*)&beta,
                A, //(float_complex_t *) (myM + (k * msbs2)),
                ms,
...
void taskGen(Parameters<ff_mdf>* const P)
{
    ...
    for (int k = 0; k <= (nb - 1); k++)
    {
        ...
        {
            Param.clear();
            const param_info _1 = {(uintptr_t)(&A[k * bs * bs * nb + k * bs]),
                                   INPUT};
            const param_info _2 = {(uintptr_t)(&A[k * bs * bs * nb + i * bs]),
                                   INPUT};
            const param_info _3 = {(uintptr_t)(&A[k * bs * bs * nb + k * bs]),
                                   OUTPUT};
            Param.push_back(_1);
            Param.push_back(_2);
            Param.push_back(_3);
            mdf->AddTask(Param, F_CHERK, &A[k * bs * bs * nb + k * bs],
                         &A[k * bs * bs * nb + i * bs],
                         &A[k * bs * bs * nb + k * bs]);
            ++numtasks;
        } // for i
        ...
        const param_info _1 = {(uintptr_t)(&A[k * bs * bs * nb + k * bs]),
                               INPUT};
        const param_info _2 = {(uintptr_t)(&A[k * bs * bs * nb + k * bs]),
                               OUTPUT};
        Param.push_back(_1);
        Param.push_back(_2);
        mdf->AddTask(Param, F_CPOTF2, &A[k * bs * bs * nb + k * bs],
                     &A[k * bs * bs * nb + k * bs]);
        ++numtasks;
        for (int j = (k + 1); j <= (nb - 1); j++)
        {
            for (int i = 0; i <= (k - 1); i++)
            {
                Param.clear();
                const param_info _1 = {
                    (uintptr_t)(&A[k * bs * bs * nb + i * bs]), INPUT};
                const param_info _2 = {
                    (uintptr_t)(&A[j * bs * bs * nb + i * bs]), INPUT};
                const param_info _3 = {
                    (uintptr_t)(&A[j * bs * bs * nb + k * bs]), INPUT};
                const param_info _4 = {
                    (uintptr_t)(&A[j * bs * bs * nb + k * bs]), OUTPUT};
                Param.push_back(_1);
                Param.push_back(_2);
                Param.push_back(_3);
                Param.push_back(_4);
                mdf->AddTask(Param, F_CGEMM, &A[k * bs * bs * nb + i * bs],
                             &A[j * bs * bs * nb + i * bs],
                             &A[j * bs * bs * nb + k * bs],
                             &A[j * bs * bs * nb + k * bs]);
                ++numtasks;
            ...
            const param_info _1 = {(uintptr_t)(&A[k * bs * bs * nb + k * bs]),
                                   INPUT};
            const param_info _2 = {(uintptr_t)(&A[j * bs * bs * nb + k * bs]),
                                   INPUT};
            const param_info _3 = {(uintptr_t)(&A[j * bs * bs * nb + k * bs]),
                                   OUTPUT};
            Param.push_back(_1);
            Param.push_back(_2);
            Param.push_back(_3);
            mdf->AddTask(Param, F_CTRSM, &A[k * bs * bs * nb + k * bs],
                         &A[j * bs * bs * nb + k * bs],
                         &A[j * bs * bs * nb + k * bs]);
            ++numtasks;
        } // for j
...
int main(int argc, char* argv[])
{
    ...
    for (int j = 0; j < 1; ++j)
    {
        ...
        fprintf(fp,
                "Block algorithm farm version on Intel: L result matrix \n");

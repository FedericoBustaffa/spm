...
public:
	...
	Resource(const std::vector<T>& data) : data_(data) { std::cout << "Resource constructed\n"; }
...
// std::function
// ---------------------------------------------------
void print(const std::string& s) { std::printf("%s\n", s.c_str()); }
std::vector<std::function<void(const std::string&)>> F = {
	print, [](const std::string& s) { std::cout << s << " from lambda\n"; }};

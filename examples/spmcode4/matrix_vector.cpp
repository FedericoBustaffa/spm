...
template <typename value_t, typename index_t>
void sequential_mult(std::vector<value_t>& A, std::vector<value_t>& x,
...
template <typename value_t, typename index_t>
void block_parallel_mult(std::vector<value_t>& A, std::vector<value_t>& x,
                         std::vector<value_t>& b, index_t m, index_t n,
...
template <typename value_t, typename index_t>
void block_parallel_mult2(std::vector<value_t>& A, std::vector<value_t>& x,
                          std::vector<value_t>& b, index_t m, index_t n,
                          index_t num_threads = 32)
{
...
template <typename value_t, typename index_t>
void block_cyclic_parallel_mult(std::vector<value_t>& A,
                                std::vector<value_t>& x,
                                std::vector<value_t>& b, index_t m, index_t n,
                                index_t num_threads, index_t chunk_size)
{
...
void check(std::vector<value_t>& b, index_t m, index_t n)
{
    for (uint64_t index = 0; index < m; index++)
    {
        ...
        if (index < n)
            ...
            val = n * (n - 1) / 2;
        if (b[index] != val)
            std::cout << "error at position " << index << " " << b[index]
                      << std::endl;
    }
}

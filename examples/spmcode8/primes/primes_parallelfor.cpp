...
int main(int argc, char* argv[])
{
    ...
    {
        std::cerr << "use: " << argv[0]
                  << " number1 number2 nworkers [chunk=0] [print=off|on]\n";
        return -1;
    }
    ...
    if (chunk != 0)
    { // in case of dynamic scheduling we have to order the array
        for (size_t i = 0; i < nworkers; ++i)
        {
            if (R[i].size())
            {
                results.insert(
                    std::upper_bound(results.begin(), results.end(), R[i][0]),
                    R[i].begin(), R[i].end());
            }
        }

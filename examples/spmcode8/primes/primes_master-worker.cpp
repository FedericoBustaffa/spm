...
//
//  -   The Emitter generates as many sub-ranges as the number of (Workers-1)
//  keeping a
//      sub-range for itself to compute locally. After it has computed the local
//      sub-range it waits for to receive the primes in the sub-range assigned
//      to each Worker.
//  -   Each Worker finds primes within its sub-range and then sends back to the
//  Emitter
...
struct Emitter : ff_monode_t<Task_t>
{
    ...
    Emitter(ull n1, ull n2) : n1(n1), n2(n2)
    {
    ...
    Task_t* svc(Task_t* in)
    {
        if (in == nullptr)
        {                                         // first call
            const int nw = get_num_outchannels(); // gets the total number of
                                                  // workers added to the farm
        ...
        if (V.size()) // We may receive an empty vector
            results.insert(
                std::upper_bound(results.begin(), results.end(), V[0]),
                V.begin(), V.end());
        delete in;
        return GO_ON;
    }
...
int main(int argc, char* argv[])
{
    ...
    {
        std::cerr << "use: " << argv[0]
                  << " number1 number2 nworkers [print=off|on]\n";
        return -1;
    }
    ...
    else
    {
        ...
        farm.wrap_around(); // adding feedback channels between Workers and the
                            // Emitter

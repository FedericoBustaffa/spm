...
double product(const double* matrix, const int n, const int row, const int col)
{
    double prod = 0.0;
    int len = col - row + 1;
...
void compute_iter(double* matrix, const int n, const int start, const int stop,
                  const int iter)
{
...
struct Master : ff_monode_t<task_t>
{
    ...
    void send_tasks(int& start, int& stop)
    {
        ...
        nsent = (pardegree - 1);
        // if there are more Worker than task, terminate one Worker (the last
        // one)
        if ((n - iter) == pardegree)
        {
...
void wavefront(double* matrix, int n, int pardegree, bool blocking,
               bool nomapping)
{
    if (pardegree == 1)
    ...
    if (nomapping)
        farm.no_mapping(); // reset thread affinity
...
int main(int argc, char** argv)
{
    ...
    if (argc < 3)
    {
        ...
        std::printf("example: %s 1024 10 1 1    # means BLOCKING_MODE and "
                    "NO_DEFAULT_MAPPING\n",
                    argv[0]);
        return -1;
    }
    ...
    std::cout << "Matrix size: " << n
              << " Time taken: " << duration.count() / 1000.0
              << "ms Final cell: " << matrix[n - 1] << "\n";

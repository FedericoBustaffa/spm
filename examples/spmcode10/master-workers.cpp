...
int main(int argc, char* argv[])
{
    ...
    {
        int n_eos = 0;
        std::cout << "Hello I'm the server with id " << myid << " on "
                  << processor_name << "\n";
        ...
        while (true)
        {
            ...
            else
            {
                std::cout << "[server] Request from " << status.MPI_SOURCE
                          ...
                          << "\n";
                target *= target;
                MPI_Send(&target, 1, MPI_INT, status.MPI_SOURCE, msg_tag,
                         MPI_COMM_WORLD);
            }
        }
    }
    ...
    { // This is the Worker code
        ...
        while (rep < noreq)
        {
            ...
            MPI_Recv(&request, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD,
                     &status);
            std::cout << "[Worker" << myid << "] <- " << request << "\n";
            usleep(random() / 4000);
            ++rep;
        }
        std::cout << "** Worker" << myid << " received " << noreq
                  << " answers\n";
    ...
    std::cout << "Total time       " << myid << " is "
              << diffmsec(wt1, wt0) / 1000 << " (S)\n";
    return 0;
}

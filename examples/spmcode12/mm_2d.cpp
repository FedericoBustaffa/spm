 ...
 *   global m * k and k * n matrices into sqrt(P) * sqrt(P) blocks across P MPI
 * processes. Process 0:
...
void printOutput(int rows, int cols, float* data)
{
    ...
    if (fp == NULL)
    {
        std::cout << "ERROR: Output file out2D.txt could not be opened"
                  << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    if ((p.m % gridDim) || (p.n % gridDim))
    {
        ...
        if (!myId)
            std::cout
                << "ERROR: 'm', 'n' must be multiple of the grid dimensions"
                << std::endl;
    ...
    MPI_Scatterv(A, sendcounts_A.data(),
                 displs_A.data(), // 0,0,1,1 for gridDim=2
                 rowsType, myA, blockRows * p.k, MPI_FLOAT, 0, MPI_COMM_WORLD);
    ...
    MPI_Type_create_resized(colsType, lb, blockCols * sizeof(float),
                            &colsTypeResized);
    MPI_Type_commit(&colsTypeResized);
    MPI_Type_free(&colsType);

    myB = new float[p.k * blockCols];
    // prepare Scatterv for B: each rank receives block column j = rank %
    // gridDim
    ...
    MPI_Scatterv(B, sendcounts_B.data(), displs_B.data(), colsTypeResized, myB,
                 p.k * blockCols, MPI_FLOAT, 0, MPI_COMM_WORLD);
    ...
    MPI_Gatherv(myC, blockRows * blockCols, MPI_FLOAT, rootBuf,
                recvcounts_C.data(), displs_C.data(), block2DTypeResized, 0,
                MPI_COMM_WORLD);

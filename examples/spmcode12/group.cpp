 ...
 *
 * Description:
 *   - Initializes MPI and obtains global rank ('myrank') and size ('size') of
 * MPI_COMM_WORLD.
 ...
 *   - Uses MPI_Group_incl to build 'new_group' containing only the ranks in the
 * lower or upper half, depending on 'myrank'.
 *   - Determines each process’s rank within 'new_group' with MPI_Group_rank.
 *   - Creates a new communicator 'new_comm' from 'new_group' via
 * MPI_Comm_create.
 *   - Performs an MPI_Allreduce over 'new_comm' to sum the original world-ranks
 * ('myrank') across each subgroup.
 *   - Each process prints its original rank, its rank within the new
 * communicator, and the sum of original ranks in its subgroup.
 *
 */

 ...
 *     sub‐blocks of A and B to all P processes arranged in a sqrt(P) * sqrt(P)
 * grid via non-blocking MPI_Isend and MPI_Recv.
 ...
 *
 * Notes:
 *   - SUMMA requires works for any sqrt(P) grid and matrix dimensions divisible
 * by sqrt(P)
 *   - Possible optimization (not implemented): broadcasts overlap communication
 * with local computation across steps (needs double buffering).
 */
#include "mpi.h"
#include <cmath>
#include <cstdio>
...
void printOutput(int rows, int cols, float* data)
{
    ...
    if (fp == NULL)
    {
        std::cout << "ERROR: Output file outSUMMA.txt could not be opened"
                  << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    if ((m % gridDim) || (n % gridDim) || (k % gridDim))
    {
        ...
        if (!myId)
            std::cout
                << "ERROR: 'm', 'k' and 'n' must be multiple of sqrt(numP)"
                << std::endl;
    ...
    if (!myId)
    {
        for (int i = 0; i < gridDim; i++)
        {
            ...
            {
                MPI_Isend(A + i * blockRowsA * k + j * blockRowsB, 1,
                          blockAType, i * gridDim + j, 0, MPI_COMM_WORLD, &req);
                MPI_Isend(B + i * blockRowsB * n + j * blockColsB, 1,
                          blockBType, i * gridDim + j, 0, MPI_COMM_WORLD, &req);
            }
        }
    }

    MPI_Recv(myA, blockRowsA * blockRowsB, MPI_FLOAT, 0, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
    MPI_Recv(myB, blockRowsB * blockColsB, MPI_FLOAT, 0, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
    ...
    for (int i = 0; i < gridDim; i++)
    {
        ...
        for (int i = 0; i < blockRowsA; i++)
        {
            for (int j = 0; j < blockColsB; j++)
            {
                ...
                {
                    myC[i * blockColsB + j] +=
                        buffA[i * blockRowsB + l] * buffB[l * blockColsB + j];
                }
            }
        }
    }
    ...
    if (!myId)
    {
        ...
        for (int i = 0; i < gridDim; i++)
            for (int j = 0; j < gridDim; j++)
                if (i || j)
                    MPI_Recv(&C[i * blockRowsA * n + j * blockColsB], 1,
                             blockCType, i * gridDim + j, 0, MPI_COMM_WORLD,
                             MPI_STATUS_IGNORE);
    }
    else

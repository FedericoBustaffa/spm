 ...
 *   - Computes a 'color' for each process as (2*myrank)/numtasks, splitting all
 ...
 *   - Calls MPI_Comm_split to create a new communicator (new_comm) for each
 * group.
 ...
 *     across all processes in the same color group, storing the result in
 * `rcvbuf`.
 *   - Each process prints its original rank, its newrank in the split
 * communicator, and the sum of original ranks in its group.
 *
 */

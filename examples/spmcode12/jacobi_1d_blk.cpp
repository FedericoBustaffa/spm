...
int main(int argc, char* argv[])
{
    ...
    while (error > errThres)
    {
        if (myId > 0)
        {
            ...
            MPI_Sendrecv(myData, cols, MPI_FLOAT, myId - 1, 0, prevRow, cols,
                         MPI_FLOAT, myId - 1, 1, MPI_COMM_WORLD,
                         MPI_STATUS_IGNORE);
        }
        ...
        if (myId < numP - 1)
        {
            ...
            MPI_Sendrecv(&myData[(myRows - 1) * cols], cols, MPI_FLOAT,
                         myId + 1, 1, nextRow, cols, MPI_FLOAT, myId + 1, 0,

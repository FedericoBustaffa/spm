// Run with:    mpirun -n <num_procs> ./power_iteration <matrix_size>
// <max_iters> <tollerance> Example:     mpirun -np 4 ./power_iter 800 1000 1e-8
//
...
//
// The program approximates the dominant eigenvalue (lambda_max) and eigenvector
// (v) until ||v_{k} − v_{k-1}|| < tollerance or max_iters iterations are
// reached.
//
#include <mpi.h>
...
int main(int argc, char** argv)
{
    ...
    if (rank == 0)
    {
        std::cout << "Power iteration on inverse-distance matrix of size " << N
                  << "x" << N << " (up to " << max_iters
    ...
    for (int iter = 0; iter < max_iters; ++iter)
    {
        ...
        MPI_Allgatherv(Ax_local.data(), local_rows, MPI_DOUBLE, Ax.data(),
    ...
    {
        std::cout << "----------------------------------------\n"
                  << "Dominant eigenvalue ~= " << std::setprecision(12)
                  << lambda << "\n";
    }
    MPI_Finalize();
    return 0;

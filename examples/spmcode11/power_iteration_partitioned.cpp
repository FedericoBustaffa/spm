...
inline double Aij(int i, int j) { return 1.0 / (1.0 + std::abs(i - j)); }
...
int main(int argc, char** argv)
{
    ...
    {
        MPI_Scatterv(nullptr, nullptr, nullptr, MPI_DOUBLE, x_local.data(),
                     local_rows, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    }

    // Normalise initial vector globally so that ||x_0|| = 1
    double local_n2 = std::inner_product(x_local.begin(), x_local.end(),
                                         x_local.begin(), 0.0);
    double global_n2;
    MPI_Allreduce(&local_n2, &global_n2, 1, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
    double inv_norm = 1.0 / std::sqrt(global_n2);
    for (double& val : x_local)
        val *= inv_norm;

    if (rank == 0)
        std::cout << "Partitioned power iteration: n=" << n
    ...
    for (int iter = 0; iter < max_iters; ++iter)
    {
        ...
        MPI_Allgatherv(x_local.data(), local_rows, MPI_DOUBLE, x_full.data(),
        ...
        // Rayleigh quotient lambda_k = (x_k * y_k) / (x_k * x_k) (NOTE:the
        // denominator is 1)
        double local_num = 0.0;
        for (int i = 0; i < local_rows; ++i)
            local_num += x_local[i] * Ax_local[i];
        MPI_Allreduce(&local_num, &lambda, 1, MPI_DOUBLE, MPI_SUM,
                      MPI_COMM_WORLD);

        // Normalise Ax_local
        double local_y2 = std::inner_product(Ax_local.begin(), Ax_local.end(),
                                             Ax_local.begin(), 0.0);
        double global_y2;
        MPI_Allreduce(&local_y2, &global_y2, 1, MPI_DOUBLE, MPI_SUM,
                      MPI_COMM_WORLD);
        double inv_y_norm = 1.0 / std::sqrt(global_y2);
        for (double& val : Ax_local)
            val *= inv_y_norm;
        ...
        double global_diff2;
        MPI_Allreduce(&local_diff2, &global_diff2, 1, MPI_DOUBLE, MPI_SUM,
                      MPI_COMM_WORLD);

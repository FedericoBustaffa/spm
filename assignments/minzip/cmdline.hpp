...
static inline void usage(const char* argv0)
{
    ...
    std::printf("Usage: %s [options] file-or-directory [file-or-directory]\n",
                argv0);
    std::printf("\nOptions:\n");
    std::printf(" -r 0 does not recur, 1 will process the content of all "
                "subdirectories (default r=%d)\n",
                RECUR ? 1 : 0);
    std::printf(" -C compress: 0 preserves, 1 removes the original file "
                "(default C=%d)\n",
                REMOVE_ORIGIN && COMP ? 1 : 0);
    std::printf(" -D decompress: 0 preserves, 1 removes the original file "
                "(default D=%d)\n",
                REMOVE_ORIGIN && !COMP ? 1 : 0);
    std::printf(" -q 0 silent mode, 1 prints only error messages to stderr, 2 "
                "verbose (default q=%d)\n",
                QUITE_MODE);
    std::printf("--------------------\n");
}
...
int parseCommandLine(int argc, char* argv[])
{
    ...
    if ((argc - start) <= 0)
    {
        std::fprintf(
            stderr,
            "Error: at least one file or directory should be provided!\n");
        usage(argv[0]);
        return -1;
    }

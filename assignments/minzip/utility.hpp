                                ...
                                unsigned char*& ptr)
{
    ...
    ptr = (unsigned char*)mmap(nullptr, size, PROT_READ | PROT_WRITE,
                               MAP_SHARED, fd, 0);
    if (ptr == MAP_FAILED)
    {
        if (QUITE_MODE >= 1)
                                  ...
                                  const std::string& fname)
{
    ...
    // allocate the space in a file for the uncompressed data. The file is
    // memory mapped.
        ...
        else
        {
            if (discardIt(file->d_name, comp))
            {
                if (QUITE_MODE >= 2)
                {
                    ...
                    {
                        std::fprintf(stderr,
                                     "%s has already a %s suffix -- ignored\n",
                                     file->d_name, SUFFIX);
                    }
                    else
                    {
                        std::fprintf(
                            stderr, "%s does not have a %s suffix -- ignored\n",
                            file->d_name, SUFFIX);
                    }
                }
                continue;
